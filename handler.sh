...
_preconfigure() {
  if [[ -n "${CONFIG_ENDPOINT}" ]]; then
    ...
    jq -r "${CONFIG_SELECTOR}" /tmp/config.json | jq -r 'to_entries|map("\(.key)=\(.value|tostring)")|.[]' >/tmp/vars.txt
...
_notify() {
  for var_name in $(env | grep WEBHOOK_ENDPOINT | grep -v TEMPLATE | awk -F"=" '{print $1}'); do
    template_name="${var_name}_TEMPLATE"
    if [[ -n ${!var_name} ]] && [[ -n ${!template_name} ]]; then
      ...
      notification_result=$?
      if [[ $notification_result -ne 0 ]]; then
...
_execute() {
  if [[ ! -f ${script_dir}/download-${cmd}-upload.sh ]]; then
